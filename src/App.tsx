    ...
    <div className='relative flex items-center justify-center w-full h-screen overflow-hidden px-2 py-32 bg-center bg-cover bg-no-repeat'
      ...
      <div className='flex items-center flex-col justify-evenly w-full h-full bg-transparent backdrop-blur border-2 border-t-white/30 border-r-white/30 border-b-white/20 border-l-white/10 rounded-2xl px-8 transition-all '>
        ...
        <div className={`flex-col justify-evenly gap-2 p-5 ${click === false ? 'hidden' : 'flex'}`}>
          ...
          <div>
            <h1 className='text-4xl font-bold text-red-600'>Oi, gatinha!</h1>
            <p className='text-xl font-medium text-white/80 text-justify'>Fiz isso aqui só para te lembrar nesse dia dos namorados, que você é muito, muito, muito especial. Eu sou muito feliz de ter você todos os dias comigo, você é a prova do amor e cuidado de Deus para mim e é por você que não desisto e quero cada dia ser melhor. Te amo e te quero para sempre e sempre!</p>
          </div>
          <div className='flex items-center justify-center gap-5 transition-all'>
            
          </div>
        </div>
        <button onClick={() => setClick(true)} className={`bg-red-700/60 shadow-2xl shadow-red-500/50 text-white text-2xl font-bold p-5 rounded-2xl ${click ? 'hidden' : ''}`}>Clique aqui!</button>
        <button onClick={() => setClick(false)} className={`flex items-center justify-center gap-3 bg-red-700/60 shadow-2xl shadow-red-500/50 text-white text-2xl font-bold p-5 rounded-2xl ${click ? '' : 'hidden'}`}>
TE AMO!
